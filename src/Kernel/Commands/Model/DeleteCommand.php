...
class DeleteCommand extends Command
{
    protected function configure(): void
    {
        ...
        $this->setAliases(['rm:model']);
        $this->setDescription('Delete a model from an application');
        $this->addArgument('app', InputArgument::REQUIRED, 'The name of the application');
        $this->addArgument('name', InputArgument::REQUIRED, 'The name of the model to delete');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!$modelRepository->exists($app, $name)) {
            $output->writeln("<error>Model $name does not exist in $app</error>");
            return Command::FAILURE;
        }

        if (!$modelRepository->delete($app, $name)) {
            $output->writeln("<error>Failed to delete model $name from $app</error>");
            return Command::FAILURE;
        }

        $output->writeln("<info>Model $name deleted from $app</info>");
